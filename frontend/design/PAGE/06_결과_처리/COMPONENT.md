...
### 진입 경로 (2가지)
```
1. 검사 이력 [탭2] → **모든 카드** → 정밀 분석 모달 → "결과 처리" 버튼 → 결과 처리 (최초 저장 또는 재수정)
   (양품 자동종결분도 진입 가능 — 오분류 정정용. C-B로 모든 결과에 서버 대표 행이 있으므로 서버 결과 대표 행에 피드백)
2. Bottom Nav [탭3] → 결과 처리 직접 접근
```
...
| 본문 상단 | 검사 요약 카드 | Card (Row) | 썸네일(20×20) + "AI 판정: [결함유형]" + "신뢰도: [%]" — 서버 분석분=서버 결과, 단말 자동종결분=단말 결과(`server_result ?? device_result`) |
| 본문 | 추론 결과 (Top-3) | List | 제목 "추론 결과 (Top-3 Predictions)" + 1~3위 프로그레스 바 (모달과 동일 데이터, `server_result ?? device_result`의 상위_예측). 빈 배열이면 섹션 숨김 |
...
### 6.1 워크플로우 A: 최초 저장 (검사 이력에서 카드 진입)
```
검사 이력 [탭2] → 카드 (모든 유형) → 정밀 분석 모달
    │
    ▼
